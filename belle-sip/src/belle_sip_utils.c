...
char * belle_sip_concat (const char *str, ...) {
  ...
  char *result = (char *) ortp_malloc (allocated);
  ...
  if (result != NULL)
    {
          ...
          if (wp + len + 1 > result + allocated)
            {
              ...
              newp = (char *) ortp_realloc (result, allocated);
              if (newp == NULL)
                {
                  ortp_free (result);
                  return NULL;
      ...
      *wp++ = '\0';
      ...
      newp = ortp_realloc (result, wp - result);
      if (newp != NULL)
        result = newp;

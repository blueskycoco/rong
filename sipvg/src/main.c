...
void on_sip_session_close(sip_voice_service_t* s){
...
int on_session_created(sip_voice_service_t* s,char* caller){
	return 0;
}
...
int main(int argc,const char** argv){
  ...
  sip_voice_service_set_audio_data_cb(vg,on_sip_audio_data,on_sip_session_close,
	on_session_created);  
  while(1){
	usage();
	char ch=getchar();
	if(ch=='h'){
	  ...
	  terminate_call(vg);
	   ...
	   printf("please input the record filename:");
           char filename[100]={0};
           scanf("%s",filename);
           if(strcmp(filename,"")==0)strcpy(filename,DEFAULT_PCM_FILE);
	   if(ta){
		test_audio_file_stop(ta);
	        test_audio_file_destroy(ta);
	   }

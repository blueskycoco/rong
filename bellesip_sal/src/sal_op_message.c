...
//conference query
int sal_conf_query_message_send(SalOp *op, const char *from, const char *to, const char* comtype,const char* subject){
	belle_sip_request_t* req;
	...
	//support h239
	if(comtype)
		belle_sip_message_add_header(BELLE_SIP_MESSAGE(req),belle_sip_header_create( "comType", comtype));
	if(subject)
		belle_sip_message_add_header(BELLE_SIP_MESSAGE(req),belle_sip_header_create( "Subject", subject));

	belle_sip_message_add_header(BELLE_SIP_MESSAGE(req),BELLE_SIP_HEADER(belle_sip_header_content_length_create(content_length)));
	belle_sip_message_add_header(BELLE_SIP_MESSAGE(req),BELLE_SIP_HEADER(belle_sip_header_date_create_from_time(&curtime)));
	belle_sip_message_set_body(BELLE_SIP_MESSAGE(req),NULL,content_length);
	return sal_op_send_request(op,req);

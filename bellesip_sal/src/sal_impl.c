...
void sal_set_callbacks(Sal *ctx, const SalCallbacks *cbs){
	...
	//suport pres 
	if(ctx->callbacks.on_pres_req_answered==NULL)
		ctx->callbacks.on_pres_req_answered=(SalOnExpire)unimplemented_stub;
	if(ctx->callbacks.on_pres_reqest==NULL)
		ctx->callbacks.on_pres_reqest=(SalOnExpire)unimplemented_stub;
}
...
void sal_get_default_local_ip(Sal *sal, int address_family, char *ip, size_t iplen){
	...
	const char* ipaddr=inet_ntoa(sin.sin_addr);
	strcpy(ip,ipaddr);  
	close(sock);      
}

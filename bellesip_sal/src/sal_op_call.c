...
static void process_request_event(void *op_base, const belle_sip_request_event_t *event) {
	...
	case BELLE_SIP_DIALOG_CONFIRMED:
		...
		if (op->reinvite){
			...
			const char* mb=belle_sip_message_get_body(BELLE_SIP_MESSAGE(req));
			//recieved h239 request
			if(mb&&strstr(mb,"pres_token_request")) {
				resp=sal_op_create_response_from_request(op,req,200);
				belle_sip_server_transaction_send_response(server_transaction,resp);

				if (op->base.root->callbacks.on_pres_reqest){
					op->base.root->callbacks.on_pres_reqest(op,1);
				}	
			}else{
				//白板请求回应
				if(mb&&strstr(mb,"pres_token_response")) {
					//增加回调通知应用层
					ms_message("Receiving pres request on op [%p]",op);
					if (op->base.root->callbacks.on_pres_req_answered){
						op->base.root->callbacks.on_pres_req_answered(op,1);
					}				
				}//释放白板
				else if(mb&&strstr(mb,"pres_token_release")) {
					if (op->base.root->callbacks.on_pres_reqest){
						op->base.root->callbacks.on_pres_reqest(op,0);
					}
				}//拥有白板
				else if(mb&&strstr(mb,"pres_token_owner")) {
					if (op->base.root->callbacks.on_pres_reqest){
						op->base.root->callbacks.on_pres_reqest(op,2);
					}
				}
				//白板I帧
				else if (mb&&strstr&&(mb,"presentation_token_control")&&strstr(mb,"picture_fast_update")) {
					if (op->base.root->callbacks.on_pres_reqest){
						op->base.root->callbacks.on_pres_reqest(op,3);
					}
				}
				//I帧
				else if (mb&&strstr(mb,"picture_fast_update")) {
					/*vfu request*/
